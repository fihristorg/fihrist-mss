...
<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="cat" select="'Fihrist'"/>
  ...
  <xsl:variable name="path">
    <xsl:value-of
      select="concat('file:///home/jamesc/git/fihrist-mss/working/new/?select=', $files,';on-error=warning;recurse=',$recurse)"
    />
  </xsl:variable>
  ...
  <xsl:template name="main">
    ...
    <xsl:for-each select="$doc">
      ...
      <xsl:variable name="outputFilename"
        select="concat('file:///home/jamesc/git/fihrist-mss/working/draft-updated/', $msID, '.xml')"/>
      <!-- create output file -->
...
<!-- The WMS Arabic files have the msDesc in the <body> -->
  <xsl:template match="sourceDesc"><xsl:choose>
  ...
  <xsl:template match="body[div/msDesc]">
    <xsl:copy>
      <p><xsl:comment>Body paragraph provided for validation and future transcription</xsl:comment></p>
    </xsl:copy>
  </xsl:template>
...
<xsl:value-of select="normalize-space($value)"/>  
  ...
  <xsl:function name="jc:checkEmpty" as="item()*">
    <xsl:param name="altIdentifier" as="item()"/>
    <xsl:choose>
      <xsl:when
        test="$altIdentifier/idno[@type='SCN' and not(contains(., 'Not in SC')) and contains(., ',')] | $altIdentifier/idno[@type='TM']">
