...
}
{
    ...
    for $key in distinct-values($allinstances/key)
        return if (not(some $entryid in $authorityentries/@xml:id/data() satisfies $entryid eq $key)) then
            bod:logging('warn', 'Key attribute not found in authority file: will create broken link', ($key, $allinstances[key = $key]/title, distinct-values($allinstances[key = $key]/file), $allinstances[key = $key]/link))
        else
            ()
}
{
    ...
    for $work in $authorityentries
        ...
        return if (count($instances) gt 0) then
            <doc>
                ...
                let $relatedids := tokenize(replace(string-join(($work/@corresp, $work/@sameAs), ' '), '\S*#', ''), '\s+')[string-length() gt 0]
