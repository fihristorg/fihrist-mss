    ...
    <xsl:variable name="viaf2isni" as="map(xs:string, node())">
        <xsl:map>
            <xsl:for-each select="$isniapiresults">
                ...
                <xsl:if test="$isnidoc/ZiNG:numberOfRecords[1]/text() ne '0'">
                    <xsl:if test="$isnidoc/ZiNG:numberOfRecords[1]/text() eq '1'">
                        ...
                        <xsl:if test="$isniiddatafield/marc:subfield[@code='a']/text()[1] = 'assigned'">
